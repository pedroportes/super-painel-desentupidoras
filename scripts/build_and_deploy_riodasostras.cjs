...
const cityIndex = cities.findIndex(c => c.id === 'riodasostras');

if (cityIndex === -1) {
  console.error('Cidade riodasostras não encontrada em cities.json');
  process.exit(1);
}
...
function syncCityToAstro(cityData) {
  ...
  const astroConfig = {
    ...
    uf: cityData.uf,
    populacao: cityData.populacao || '168.455',
    ddd: cityData.whatsapp ? cityData.whatsapp.substring(0, 2) : '22',
    ...
    aboutCityText: cityData.aboutCityText || `${cityData.cidade} é um dos municípios mais dinâmicos de ${cityData.uf}.`,
  ...
  console.log('✅ Sincronizado cityConfig.json com variants e SEO.');
}

async function main() {
  console.log('🚀 Iniciando 1º Build para riodasostras...');

...
const cityIndex = cities.findIndex(c => c.id === 'tiangua');

if (cityIndex === -1) {
  console.error('Cidade tiangua não encontrada em cities.json');
  process.exit(1);
}
...
function syncCityToAstro(cityData) {
  ...
  const astroConfig = {
    ...
    estado: 'Ceará',
    uf: cityData.uf,
    populacao: cityData.populacao || '86.968',
    ddd: cityData.whatsapp ? cityData.whatsapp.substring(0, 2) : '88',
    ...
    geoCoordinates: {
      latitude: cityData.latitude || '-3.7322',
      longitude: cityData.longitude || '-40.9931'
    },
    seo: {
      metaTitle: cityData.metaTitle || `Desentupidora em ${cityData.cidade} CE | 24 Horas Chegada Rápida`,
      metaDescription: cityData.metaDescription || `Desentupidora em ${cityData.cidade} CE com atendimento 24h para esgotos, pias, ralos e vasos. Equipe técnica local com chegada rápida e orçamento gratuito sem taxa!`,
      h1Title: cityData.h1Title || `Desentupidora em ${cityData.cidade} CE 24 Horas Especializada`,
      firstParagraphText: cityData.firstParagraph || `Procurando desentupidora em ${cityData.cidade} CE com atendimento urgente? Nossa equipe especializada atende 24 horas por dia em todos os bairros da Serra da Ibiapaba com chegada rápida e orçamento gratuito sem taxa.`,
      lastH2Title: cityData.lastH2 || `Por que Chamar Nossa Desentupidora em ${cityData.cidade} CE?`
    },
    aboutCityTitle: cityData.aboutCityTitle || `Desentupidora em ${cityData.cidade} - ${cityData.uf}`,
    aboutCityText: cityData.aboutCityText || `${cityData.cidade} é o principal polo da Serra da Ibiapaba e do Noroeste Cearense.`,
    cityFacts: [
      'Capital da Serra da Ibiapaba e polo distribuidor da Ceasa regional para todo o Nordeste.',
      'Localizada a mais de 700 metros de altitude com clima ameno e forte produção agrícola.',
      'Necessidade de desobstrução preventiva regular devido à condensação de gorduras no clima serrano.'
    ],
    citySources: ['IBGE', 'Prefeitura de Tianguá'],
    bairroEvidence: {},
    neighborhoodContent: {},
  ...
  console.log('✅ Sincronizado cityConfig.json com variants e SEO para Tianguá.');
}

async function main() {
  console.log('🚀 Iniciando 1º Build para tiangua...');

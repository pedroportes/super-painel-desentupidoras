...
const cityIndex = cities.findIndex(c => c.id === 'unai');

if (cityIndex === -1) {
  console.error('Cidade unai não encontrada em cities.json');
  process.exit(1);
}
...
function syncCityToAstro(cityData) {
  ...
  const astroConfig = {
    ...
    estado: 'Minas Gerais',
    uf: cityData.uf,
    populacao: cityData.populacao || '91.320',
    ddd: cityData.whatsapp ? cityData.whatsapp.substring(0, 2) : '38',
    ...
    geoCoordinates: {
      latitude: cityData.latitude || '-16.3575',
      longitude: cityData.longitude || '-46.9056'
    },
    seo: {
      metaTitle: cityData.metaTitle || `Desentupidora em ${cityData.cidade} MG | 24 Horas Chegada Rápida`,
      metaDescription: cityData.metaDescription || `Desentupidora em ${cityData.cidade} MG com atendimento 24h para esgotos, pias, ralos e vasos. Equipe técnica local com chegada rápida e orçamento gratuito sem taxa!`,
      h1Title: cityData.h1Title || `Desentupidora em ${cityData.cidade} MG 24 Horas Especializada`,
      firstParagraphText: cityData.firstParagraph || `Procurando desentupidora em ${cityData.cidade} MG com atendimento urgente? Nossa equipe especializada atende 24 horas por dia em todos os bairros e setores industriais com chegada rápida e orçamento gratuito sem taxa.`,
      lastH2Title: cityData.lastH2 || `Por que Chamar Nossa Desentupidora em ${cityData.cidade} MG?`
    },
    aboutCityTitle: cityData.aboutCityTitle || `Desentupidora em ${cityData.cidade} - ${cityData.uf}`,
    aboutCityText: cityData.aboutCityText || `${cityData.cidade} é o principal polo do Noroeste de Minas Gerais.`,
    cityFacts: [
      'Capital do agronegócio e líder nacional na produção de grãos e sementes do Noroeste Mineiro.',
      'Banhada pelas águas do Rio Preto e polo comercial regional de grande dinamismo.',
      'Clima com estiagem e tempestades que exigem atenção com redes pluviais e caixas de gordura.'
    ],
    citySources: ['IBGE', 'Prefeitura de Unaí'],
    bairroEvidence: {},
    neighborhoodContent: {},
  ...
  console.log('✅ Sincronizado cityConfig.json com variants e SEO para Unaí.');
}

async function main() {
  console.log('🚀 Iniciando 1º Build para unai...');

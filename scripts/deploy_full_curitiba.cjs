...
function syncCityToAstro(city) {
  ...
  const astroConfig = {
    ...
    estado: 'Paraná',
    uf: city.uf,
    populacao: city.populacao || '1.773.733',
    ddd: city.ddd || '41',
    whatsapp: city.whatsapp || '41991482039',
    telefoneFixo: city.telefoneFixo || city.phone || '(41) 3224-8500',
    isDraft,
    ...
    deployUrl: city.deployUrl || 'https://desentupidora-curitiba.pages.dev',
    paletaCores: city.paletaCores || 'azul-tecnico',
    logoUrl: city.logoUrl || '/images/curitiba/logo-desentupidora-curitiba.webp',
    logoHeight: city.logoHeight || 64,
    faviconUrl: city.faviconUrl || '/images/curitiba/favicon-desentupidora-curitiba.webp',
    heroImage: city.heroImage || '/images/curitiba/desentupidora-curitiba-caminhao-limpa-fossa.webp',
    variants: {
      hero: 'HeroV4',
    ...
    geoCoordinates: {
      latitude: city.latitude || '-25.4284',
      longitude: city.longitude || '-49.2733'
    },
    seo: {
      metaTitle: city.metaTitle || `Desentupidora em ${city.cidade} ${city.uf} 24h - Atendimento Rápido`,
      metaDescription: city.metaDescription || `Desentupidora 24h em ${city.cidade} ${city.uf}. Desentupimento de esgoto, pias, ralos e hidrojateamento com orçamento grátis e chegada rápida em todos os bairros.`,
      h1Title: city.h1Title || `Desentupidora em ${city.cidade} ${city.uf} 24 Horas`,
      firstParagraphText: city.firstParagraph || `Atendimento imediato e especializado em desentupimento de esgotos, pias, ralos, vasos sanitários e limpeza de fossas com caminhão auto-vácuo em todos os bairros de ${city.cidade} e região metropolitana.`,
  ...
  console.log('✅ Sincronizado cityConfig.json e redirects para Astro (Curitiba)');
}
...
async function main() {
  ...
  const city = cities.find(c => c.id === 'curitiba');

  if (!city) {
    throw new Error('Cidade curitiba não encontrada');
  }
  ...
  console.log('✅ cities.json atualizado com sucesso para Curitiba! URL:', city.deployUrl);
}

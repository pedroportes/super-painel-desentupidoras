...
async function makeAssets() {
  const targetDir = path.join(__dirname, '..', 'apps', 'site-template-astro', 'public', 'images', 'timoteo');
  fs.mkdirSync(targetDir, { recursive: true });

  // 1. Process Hero
  const heroSrc = 'C:/Users/pedro/.gemini/antigravity-ide/brain/0de67214-52eb-45a7-a03d-0e4b3acec717/hero_timoteo_1788621498552.jpg';
  await sharp(heroSrc)
    .resize(1920, 1080, { fit: 'cover' })
    .webp({ quality: 85 })
    .toFile(path.join(targetDir, 'desentupidora-timoteo-caminhao-limpa-fossa.webp'));
  console.log('Hero Timóteo salvo!');
  ...
  <svg width="640" height="160" viewBox="0 0 640 160" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="165" y="126" font-family="'Arial Black', 'Montserrat', sans-serif" font-size="44" font-weight="900" fill="#0284c7" letter-spacing="2">TIMÓTEO</text>
  </svg>
  `);

  await sharp(logoSvg)
    .webp({ quality: 95 })
    .toFile(path.join(targetDir, 'logo-desentupidora-timoteo.webp'));
  console.log('Logo Timóteo gerado com sucesso!');
  ...
  await sharp(faviconSvg)
    .resize(128, 128)
    .webp({ quality: 95 })
    .toFile(path.join(targetDir, 'favicon-desentupidora-timoteo.webp'));
  console.log('Favicon Timóteo gerado com sucesso!');
}

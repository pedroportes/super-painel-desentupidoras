...
const cityIndex = cities.findIndex(c => c.id === 'teofilootoni');

if (cityIndex === -1) {
  console.error('Cidade teofilootoni não encontrada em cities.json');
  process.exit(1);
}
...
function syncCityToAstro(cityData) {
  ...
  const astroConfig = {
    ...
    estado: 'Minas Gerais',
    uf: cityData.uf,
    populacao: cityData.populacao || '142.851',
    ddd: cityData.whatsapp ? cityData.whatsapp.substring(0, 2) : '33',
  ...
  console.log('✅ Sincronizado cityConfig.json com variants e SEO para Teófilo Otoni.');
}

async function main() {
  console.log('🚀 Iniciando 1º Build para teofilootoni...');

...
const cityIndex = cities.findIndex(c => c.id === 'patobranco');

if (cityIndex === -1) {
  console.error('Cidade patobranco não encontrada em cities.json');
  process.exit(1);
}
...
function syncCityToAstro(cityData) {
  ...
  const astroConfig = {
    ...
    estado: 'Paraná',
    uf: cityData.uf,
    populacao: cityData.populacao || '97.821',
    ddd: cityData.whatsapp ? cityData.whatsapp.substring(0, 2) : '46',
    ...
    geoCoordinates: {
      latitude: cityData.latitude || '-26.2289',
      longitude: cityData.longitude || '-52.6706'
    },
    seo: {
      metaTitle: cityData.metaTitle || `Desentupidora em ${cityData.cidade} PR | 24 Horas Chegada Rápida`,
      metaDescription: cityData.metaDescription || `Desentupidora em ${cityData.cidade} PR com atendimento 24h para esgotos, pias, ralos e vasos. Equipe técnica local com chegada rápida e orçamento gratuito sem taxa!`,
      h1Title: cityData.h1Title || `Desentupidora em ${cityData.cidade} PR 24 Horas Especializada`,
      firstParagraphText: cityData.firstParagraph || `Procurando desentupidora em ${cityData.cidade} PR com atendimento urgente? Nossa equipe especializada atende 24 horas por dia em todos os bairros com chegada rápida e orçamento gratuito sem taxa.`,
      lastH2Title: cityData.lastH2 || `Por que Chamar Nossa Desentupidora em ${cityData.cidade} PR?`
    },
    aboutCityTitle: cityData.aboutCityTitle || `Desentupidora em ${cityData.cidade} - ${cityData.uf}`,
    aboutCityText: cityData.aboutCityText || `${cityData.cidade} é o principal polo tecnológico e de serviços do Sudoeste Paranaense.`,
    cityFacts: [
      'Polo tecnológico e de inovação do Sudoeste do Paraná, abrigando campus da UTFPR.',
      'Destaque estadual e nacional em qualidade de vida, saúde e educação.',
      'Clima subtropical com invernos rigorosos que demandam constante manutenção em redes de esgoto.'
    ],
    citySources: ['IBGE', 'Prefeitura de Pato Branco'],
    bairroEvidence: {},
    neighborhoodContent: {},
  ...
  console.log('✅ Sincronizado cityConfig.json com variants e SEO para Pato Branco.');
}

async function main() {
  console.log('🚀 Iniciando 1º Build para patobranco...');

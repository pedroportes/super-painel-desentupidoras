...
const cityIndex = cities.findIndex(c => c.id === 'itabira');

if (cityIndex === -1) {
  console.error('Cidade itabira não encontrada em cities.json');
  process.exit(1);
}
...
function syncCityToAstro(cityData) {
  ...
  const astroConfig = {
    ...
    estado: 'Minas Gerais',
    uf: cityData.uf,
    populacao: cityData.populacao || '118.053',
    ...
    geoCoordinates: {
      latitude: cityData.latitude || '-19.6178',
      longitude: cityData.longitude || '-43.2269'
    },
    seo: {
      ...
      firstParagraphText: cityData.firstParagraph || `Procurando desentupidora em ${cityData.cidade} MG com atendimento urgente? Nossa equipe especializada atende 24 horas por dia em todos os bairros da cidade com chegada rápida e orçamento gratuito sem taxa.`,
    ...
    aboutCityText: cityData.aboutCityText || `${cityData.cidade} é um dos municípios mais tradicionais da Região Central de Minas Gerais.`,
    cityFacts: [
      'Polo histórico e mundial da mineração de ferro no Brasil.',
      'Berço do ilustre poeta brasileiro Carlos Drummond de Andrade.',
      'Relevo montanhoso com morros e vales que exigem soluções técnicas precisas em redes de esgoto.'
    ],
    citySources: ['IBGE', 'Prefeitura de Itabira'],
    bairroEvidence: {},
    neighborhoodContent: {},
  ...
  console.log('✅ Sincronizado cityConfig.json com variants e SEO para Itabira.');
}

async function main() {
  console.log('🚀 Iniciando 1º Build para itabira...');

...
async function makeAssets() {
  const targetDir = path.join(__dirname, '..', 'apps', 'site-template-astro', 'public', 'images', 'luiseduardomagalhaes');
  fs.mkdirSync(targetDir, { recursive: true });

  // 1. Process Hero from a pristine clean base truck
  const baseHero = path.join(__dirname, '..', 'apps', 'site-template-astro', 'public', 'images', 'tangaradaserra', 'desentupidora-tangaradaserra-caminhao-limpa-fossa.webp');
  await sharp(baseHero)
    .resize(1920, 1080, { fit: 'cover' })
    .webp({ quality: 85 })
    .toFile(path.join(targetDir, 'desentupidora-luiseduardomagalhaes-caminhao-limpa-fossa.webp'));
  console.log('Hero Luís Eduardo Magalhães salvo!');
  ...
  <svg width="720" height="160" viewBox="0 0 720 160" xmlns="http://www.w3.org/2000/svg">
    ...
    <text x="165" y="118" font-family="'Arial Black', 'Montserrat', sans-serif" font-size="32" font-weight="900" fill="#0284c7" letter-spacing="1.2">LUÍS EDUARDO MAGALHÃES</text>
  </svg>
  `);

  await sharp(logoSvg)
    .webp({ quality: 95 })
    .toFile(path.join(targetDir, 'logo-desentupidora-luiseduardomagalhaes.webp'));
  console.log('Logo Luís Eduardo Magalhães gerado!');
  ...
  await sharp(faviconSvg)
    .resize(128, 128)
    .webp({ quality: 95 })
    .toFile(path.join(targetDir, 'favicon-desentupidora-luiseduardomagalhaes.webp'));
  console.log('Favicon Luís Eduardo Magalhães gerado!');
}

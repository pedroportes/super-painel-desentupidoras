...
const cityIndex = cities.findIndex(c => c.id === 'umuarama');

if (cityIndex === -1) {
  console.error('Cidade umuarama não encontrada em cities.json');
  process.exit(1);
}
...
function syncCityToAstro(cityData) {
  ...
  const astroConfig = {
    ...
    estado: 'Paraná',
    uf: cityData.uf,
    populacao: cityData.populacao || '123.059',
    ddd: cityData.whatsapp ? cityData.whatsapp.substring(0, 2) : '44',
    ...
    geoCoordinates: {
      latitude: cityData.latitude || '-23.7661',
      longitude: cityData.longitude || '-53.3250'
    },
    seo: {
      ...
      firstParagraphText: cityData.firstParagraph || `Procurando desentupidora em ${cityData.cidade} PR com atendimento urgente? Nossa equipe especializada atende 24 horas por dia em todas as Zonas e Parques com chegada rápida e orçamento gratuito sem taxa.`,
    ...
    aboutCityText: cityData.aboutCityText || `${cityData.cidade} é um dos municípios mais importantes do Noroeste Paranaense.`,
    cityFacts: [
      'Polo universitário, médico e comercial do Noroeste do Paraná.',
      'Conhecida carinhosamente em todo o estado como a Capital da Amizade.',
      'Malha urbana planejada com anéis concêntricos (Zonas I a VII) e grandes parques residenciais.'
    ],
    citySources: ['IBGE', 'Prefeitura de Umuarama'],
    bairroEvidence: {},
    neighborhoodContent: {},
  ...
  console.log('✅ Sincronizado cityConfig.json com variants e SEO para Umuarama.');
}

async function main() {
  console.log('🚀 Iniciando 1º Build para umuarama...');

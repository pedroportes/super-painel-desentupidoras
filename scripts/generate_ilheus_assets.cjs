...
async function run() {
  const outDir = path.join(__dirname, '..', 'apps', 'site-template-astro', 'public', 'images', 'ilheus');
  if (!fs.existsSync(outDir)) {
    fs.mkdirSync(outDir, { recursive: true });
  }

  const heroSrc = 'C:/Users/pedro/.gemini/antigravity-ide/brain/47be494f-0cf5-473e-9b60-ae48f9fd7310/hero_ilheus_1788872838800.jpg';
  await sharp(heroSrc)
    .webp({ quality: 85 })
    .toFile(path.join(outDir, 'desentupidora-ilheus-caminhao-limpa-fossa.webp'));
  ...
  const svgLogo = `<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 500 120" width="500" height="120">
    ...
    <text x="115" y="88" font-family="system-ui, -apple-system, sans-serif" font-size="28" font-weight="800" fill="#0d9488" letter-spacing="1">ILHÉUS</text>
  ...
  await sharp(Buffer.from(svgLogo))
    .webp({ quality: 90 })
    .toFile(path.join(outDir, 'logo-desentupidora-ilheus.webp'));
  ...
  await sharp(Buffer.from(svgFavicon))
    .webp({ quality: 90 })
    .toFile(path.join(outDir, 'favicon-desentupidora-ilheus.webp'));
